...
Given("I am on the Companies page", () => {
  ...
  const routePathSuffix = "/settings/companies";

  cy.url().then((currentUrl) => {
    if (!currentUrl || currentUrl.trim() === "") {
      cy.log("URL is blank!");
    } else if (!currentUrl.includes(routePathSuffix)) {
      cy.log(currentUrl);
      cy.wait(3000);
      cy.visit(Cypress.env("BASE_URL") + routePathSuffix);
    } else {
      cy.log("Already in Company page!");
    }
  });
});
...
Then("I should see a success message 'Company added successfully'", () => {
  cy.get('[data-cy="snackbar-company-add"]')
    .contains("Company created successfully.")
    .should("be.visible");
});
...
When("I click the edit button of particular company", () => {
  cy.get('[data-cy="company-ellipse-btn"]')
    .first()
    .click({ force: true })
    .wait(3000)
    .get('[data-cy="company-add-btn"]')
    .click();
});
...
When("I click the delete button of particular company", () => {
  ...
  cy.get('[data-cy="company-ellipse-btn"]')
    ...
    .click({ force: true })
    .wait(3000)
    .get('[data-cy="company-delete-btn"]')
    .click();
});
